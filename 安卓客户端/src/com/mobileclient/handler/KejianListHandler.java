...
public class KejianListHandler extends DefaultHandler {
	...
	public void characters(char[] ch, int start, int length) throws SAXException {
		...
		if (kejian != null) { 
            ...
            if ("kejianId".equals(tempString)) 
            	kejian.setKejianId(new Integer(valueString).intValue());
            else if ("title".equals(tempString)) 
            	kejian.setTitle(valueString); 
            else if ("content".equals(tempString)) 
            	kejian.setContent(valueString); 
            else if ("kejianFile".equals(tempString)) 
            	kejian.setKejianFile(valueString); 
            else if ("teacherObj".equals(tempString)) 
            	kejian.setTeacherObj(valueString); 
            else if ("uploadTime".equals(tempString)) 
            	kejian.setUploadTime(valueString); 
        } 
	}
